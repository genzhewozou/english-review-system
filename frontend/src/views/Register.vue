<template>
  <main class="auth-container" aria-labelledby="register-heading">
    <section class="auth-card" tabindex="0">
      ...
      <p class="auth-link">
        Already have an account? <router-link to="/login" aria-label="Login here">Login here</router-link>
      </p>
    ...
    async handleRegister() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.error = typeof data === 'string' ? data : (data.message || 'Registration failed');
        }
      } catch (err) {
